...
try {
    ...
    #msg -WARNING "::plugins::otel can't generate QR codes: $err"
}
...
namespace eval ::plugins::${plugin_name} {
    ...
    variable description "Upload logs to an OTel endpoint"
    ...
    proc create_ui {} {
        ...
        if { ![info exists ::plugins::otel::settings(last_upload_shot)] } {
            set ::plugins::otel::settings(last_upload_shot) {}
            set ::plugins::otel::settings(last_upload_result) {}
            set ::plugins::otel::settings(last_upload_id) {}
            set ::plugins::otel::settings(auto_upload_min_seconds) 6
            set ::plugins::otel::settings(visualizer_browse_url) "https://visualizer.coffee/shots/<ID>"
            set needs_save_settings 1
        }
        if { ![info exists ::plugins::otel::settings(last_action)] } {
            set ::plugins::otel::settings(visualizer_download_url) "https://visualizer.coffee/api/shots/<ID>"
            set ::plugins::otel::settings(last_download_result) {}
            set ::plugins::otel::settings(last_download_id) {}
            set ::plugins::otel::settings(last_download_shot_start) {}
            set ::plugins::otel::settings(last_action) {}
        }
        if { ![info exists ::plugins::otel::settings(download_by_code_url)] } {
            set ::plugins::otel::settings(download_by_code_url) "https://visualizer.coffee/api/shots/shared?code=<ID>"
        }
        if { ![info exists ::plugins::otel::settings(download_all_last_shared)] } {
            set ::plugins::otel::settings(download_all_last_shared) "https://visualizer.coffee/api/shots/shared"
        }
...
namespace eval ::plugins::${plugin_name}::otel_settings {
    ...
    proc setup { } {
        ...
        # Username
        dui add entry $page_name 280 540 -tags username -width 38 -font Helv_8  -borderwidth 1 -bg #fbfaff -foreground #4e85f4 -textvariable ::plugins::otel::settings(visualizer_username) -relief flat  -highlightthickness 1 -highlightcolor #000000 \
        ...
        # Password
        dui add entry $page_name 280 720 -tags password -width 38 -font Helv_8  -borderwidth 1 -bg #fbfaff -foreground #4e85f4 -textvariable ::plugins::otel::settings(visualizer_password) -relief flat  -highlightthickness 1 -highlightcolor #000000 \
        ...
        dui add dbutton $page_name 1350 800 -tags browse -bwidth 450 -bheight 350 -label $browse_msg \
            ...
            -command ::plugins::otel::browse -style insight_ok
    ...
    proc show { page_to_hide page_to_show } {
        if { $plugins::otel::settings(last_action) eq "download" } {
            set last_id $::plugins::otel::settings(last_download_id)
            dui item config $page_to_show last_action_label -text [translate "Last download:"]
            dui item config $page_to_show last_action -text "[translate {Shot started on }] $plugins::otel::settings(last_download_shot_start)"
            dui item config $page_to_show last_action_result -text $::plugins::otel::settings(last_download_result)
        } else {
        ...
        if { $last_id eq {} } {
            ...
            dui item show $page_to_show browse*
                ...
                zint encode [::plugins::otel::id_to_url $last_id browse] [namespace current]::qr_img -barcode QR -scale 2.6
            }
        }
    }

    proc format_shot_start {} {
        set dt $::plugins::otel::settings(last_upload_shot)
        if { $dt eq {} } {
